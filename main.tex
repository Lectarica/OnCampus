...
\titlespacing*{\section}{0pt}{5.0ex plus 1ex minus .5ex}{3.3ex plus .5ex}
...
第1章では、数値計算や解析の作業に適したプログラミング言語の紹介と簡単な文法の解説、及び環境構築の仕方について述べます。また第2章ではC++に限定して代表的なツールとその使い方について紹介します。

強相関物質の研究スタイルにはかなり決まった型が存在していますが、出発点として死活的に重要なのがハミルトニアンの構築とその対角化です。第3章では、強束縛模型について軽く説明した後、ハミルトニアンを実装する方法、及びその対角化の方法について述べます。ハミルトニアンの対角化により得られた固有値がエネルギー分散ですが、実際に研究したり論文を書いたりするうえで必要なバンド図やフェルミ面の書き方についてもここで解説します。
...
本章では数値計算に用いる言語にどのようなものがあるか、また代表的にどのような機能があるかを俯瞰した後、実際に環境構築および整備を進めていきます。すぐにでも環境構築をしたいという方は3節の「計算環境の準備」の項を読んでください。
...
実のところ筆者が一番長く触れている言語です。数値計算を補助する言語としてPython3も非常に有用です。C++やFortranに比べると、Python3はライブラリが充実していて、可視化やデータ解析を迅速に行える利点もあります。\footnote{可視化にはmatplotlib, 数値計算やデータ解析にはnumpyやscipyといったライブラリが便利でしばしば使われます。
...
Python3はインタプリタ言語であり、実行速度やメモリ制御の面ではコンパイル型言語に劣るために、大規模数値計算の中核を担うのには不適切である場合もあります\footnote{厳密には計算効率を上げるための工夫が様々に存在するようですが、その工夫に割くコストを考えると最初からC++やFortranで実装した方がよいという意見が専門家の間では散見されます。}。また型宣言がないのは実装や保守性の観点から苦しみの種となる場合もあります。しかしながら、C++やFortranで実行した本計算の出力をPythonで読み込み、プロットや解析を行うといった使い方は広く行われており、その自由度はほかの言語の追随を許しません。
...
\subsection{Makefile}
実行したいプログラムが1ファイルであれば、毎回ターミナルから手でコンパイルコマンドを打っても大した面倒ではないのですが、ソースコードが複数ファイルに分かれてきたり、複数のライブラリを併用するようになると、毎回長いコマンドを入力するのは手間がかかるうえ、オプションの付け忘れも起こりやすくなります。そこでよく用いられるのがMakefileです。

Makefileとは、どのファイルからどのように実行ファイルを作るかを記述した設定ファイルです。Makefileを作成しておけば、以後は
\begin{lstlisting}[style=terminal]
$ make
\end{lstlisting}
と入力するだけでコンパイルを実行できます。また、変更のあったファイルだけを再コンパイルしてくれるため、ソースコードが大きくなった場合にも便利です。

最も簡単なMakefileの例を以下に示します。

\begin{codebox}[Makefile]{sh}
CXX = g++
CXXFLAGS = -O3
TARGET = main
SRC = main.cpp
...
ここで、\texttt{CXX} はコンパイラ、\texttt{CXXFLAGS} はコンパイル時のオプション、\texttt{TARGET} は出力される実行ファイル名、\texttt{SRC} はソースファイル名を表しています。\\
このMakefileを \texttt{Makefile} という名前で保存しておけば、

\begin{lstlisting}[style=terminal]
$ make
 ./main
\end{lstlisting}
とするだけでプログラムを実行できます。また、

\begin{lstlisting}[style=terminal]
$ make clean
\end{lstlisting}
とすると、生成された実行ファイルを削除できます。

OpenMPやFFTW、OpenBLASなどを使う場合は、ライブラリやオプションを \texttt{CXXFLAGS} や別の変数にまとめて書くと見通しがよくなります。例えば、OpenMP, FFTW, OpenBLAS を同時に用いる場合は次のように書けます。

\begin{codebox}[Makefile]{sh}
CXX = g++
CXXFLAGS = -O3 -fopenmp
LIBS = -lfftw3_omp -lfftw3 -lopenblas -lm
TARGET = main
SRC = main.cpp
...
このようにしておくと、コンパイル条件を変更したいときも \texttt{CXXFLAGS} や \texttt{LIBS} を書き換えるだけで済みます。

ソースコードが複数ファイルに分かれている場合には、Makefileの利点がさらに大きくなります。たとえば \texttt{main.cpp}, \texttt{solver.cpp}, \texttt{util.cpp} の3つのファイルから実行ファイルを作る場合は、以下のように書けます。
...
LIBS = -lfftw3_omp -lfftw3 -lopenblas -lm
TARGET = main
SRCS = main.cpp solver.cpp util.cpp
OBJS = $(SRCS:.cpp=.o)
...
この例では、各 \texttt{.cpp} ファイルから対応する \texttt{.o} ファイルを作り、それらを最後にまとめてリンクしています。ファイル数が増えても管理しやすく、変更のあったファイルだけが再コンパイルされるため効率的です。

IntelコンパイラやMKLを用いる場合も、Makefile中の変数を書き換えるだけで対応できます。例えば、
\begin{codebox}[Makefile]{sh}
CXX = icpc
CXXFLAGS = -O3 -qopenmp -qmkl
TARGET = main
SRC = main.cpp
...
clean:
	rm -f $(TARGET)
\end{codebox}
のように書けば、MKLを利用したコンパイルを \texttt{make} で実行できます。

\section{GitHubの導入(やや発展)}
研究で書いたコードは、実験ノートのように変更履歴を後から追えると作業の面でも研究倫理の面でも望ましいのですが、実際に研究を行ってみると、ファイル名が増殖したり、最新版が分からなくなったり、どの変更でどう結果が変わったのかわからなくなったりします。また最悪のケースだと誤って上書きしてしまったり削除してしまったりする事故も起こり得ます。
ここでGitHubが便利です。GitHubとは、Git\footnote{履歴を記録しながらファイルを管理する仕組みのことと思ってください。}を用いて管理しているリポジトリをインターネット上で保管あるいは共有するためのサービスで、共同開発の現場で広く使われているものですが、個人のプロジェクトの管理においても便利です。ここでは必要最小限の機能を紹介し、リポジトリの作成方法や基本的な操作などを紹介します。

とはいえ紙面と時間の都合により詳しく述べられない部分もあるので、必要な情報は公式ドキュメントや適当な書籍などで補ってください。

\subsection{インストール}
まず\texttt{git}が有効か確認します。WSLのターミナルで
\begin{lstlisting}[style=terminal]
    $ git --version
\end{lstlisting}
と打って\texttt{git}のバージョンが表示されれば導入済みです。未インストールの場合は
\begin{lstlisting}[style=terminal]
    $ sudo apt update
...
\begin{lstlisting}[style=terminal]
    $ git commit -m "add files for RPA calculation"
\end{lstlisting}
コミットメッセージは後で見返したときにどんな変更をしたのかわかるように書くといいです。とはいえあまり長く書いてもまどろっこしいので簡潔にまとめます。

\subsection{SSH接続}
リポジトリに変更を反映すること、リポジトリの変更をローカルにも反映することはそれぞれ\texttt{git push}, \texttt{git pull}で行うことができますが、この作業には事前にGitHubとSSHを用いて通信できるようにしておく必要があります。
...
\end{lstlisting}
このコマンドを実行すると保存先及びパスフレーズ\footnote{あまり大きい声では言えないが、ここでのパスフレーズはきちんと設定しなくても大丈夫で、空のほうが作業効率は向上します。}を聞かれるので適宜入力してください。保存先は通常そのままEnterキーを押すとデフォルトで\texttt{~/.ssh/id_ed25519}に格納されます\footnote{ed25519は鍵を作るときのアルゴリズムに関連した名前らしいですが、筆者はあまり詳しくありません}。基本的にはデフォルトの場所に格納するのをお勧めします。

\subsubsection{公開鍵の登録}
次にGitHubのWebサイトに移動し、以下のように設定してください。
GitHubのマイページでアイコンをクリックすると、Settingというタブがあるので、それをクリックしてください。SSH and GPG keysからNew SSH keyをクリックします。Titleとkeyを求められますが、Titleは自由に設定してください。ssh keyの入っている端末がわかるような名前がよいでしょう。またkeyには先ほど作った公開鍵\texttt{id_ed25519.pub}の中身を貼ってください。

登録が出来たらSSH接続ができるか試してみましょう。
\begin{lstlisting}[style=terminal]
    $ ssh -T git@github.com
\end{lstlisting}
初回は接続先を信頼するかのメッセージが出ると思うので、内容を確認したうえで\texttt{yes}と入力してください。以下のようなメッセージが表示されれば成功です。
...
\subsection{リポジトリへの変更の反映}
\texttt{git push}は、ローカルで作成した変更履歴をGitHubへ送信して反映する操作です。pushで送られるのは原則コミット済の変更なので、ファイルを編集しただけではGitHubには反映されません。

個人で作業を進める場合は、きちんと\texttt{add}, \texttt{commit}, \texttt{push}していれば手元のファイルの更新とGitHub上の履歴が一致しているはずですが、会社などで集団で作業する場合は他者の変更を共有して自身の環境にも反映させる必要があるので\texttt{git pull}してリポジトリの情報をローカルに反映してください。

\subsection{その他}
ここまで紹介してきたコマンドはGitに標準搭載されているもののほんの一部に過ぎず、実際の作業現場ではブランチという作業場所の切り替え、configの設定など大量の内容があります。ここで全てを解説することは筆者の能力と時間の観点から難しいので、少なくとも\texttt{git add -A}, \texttt{git commit -m "hogehoge"}, \texttt{git push}を覚えておいてください。

\section{C++ / Python 開発環境の構築}
ここまででWSL上のUbuntuとVS Codeを連携させ、Linux環境上でプログラムを編集及び実行できる状態が整っています。以降は、本稿で扱う数値計算を行うために必要となるC++ および Pythonの開発環境を構築します。

\subsection{インストール}
まずはC++及びFortranのコンパイルに必要となる基本的なツール群を導入します。ターミナル上で以下のコマンドを実行してください。
\begin{lstlisting}[style=terminal]
...
「基礎」と銘打っていますが本章ではC++の基礎的な文法について既知とし、研究のとっかかりに最低限必要な機能とその使い方について解説します。C++の文法についてほとんど知らないという方は適宜勉強するのも良いと思いますが、ここに書いてあることをとにかく実行して手を動かしてみるのも一つの手です。なおFortranやPython、Juliaによる実行は今回は割愛いたします。
...
fftw_execute(plan);  
\end{codebox}
のように実行します。ここでplanは変換の次元、各次元のメッシュサイズ、実数か複素数かの型指定、入力・出力の配列、使用するFFTアルゴリズム(順変換か逆変換か)を指定します。なおplanを作成し終わった後にユーザーが中身を参照することはできません。
...
FFTWではデータ型および変換の種類に応じて使用するplan作成関数が異なります。主な関数は以下のように分類されます。

\begin{table}[H]
\centering
\caption{plan実行のための関数}
\renewcommand{\arraystretch}{1.3}

\begin{tabularx}{\textwidth}{|l|l|X|}
\hline
\rowcolor{tableblue!20}
\textbf{関数名} & \textbf{変換の種類} & \textbf{概要} \\
\hline

\texttt{fftw\_plan\_dft\_\{1d,2d,3d\}} &
複素数から複素数 &
複素関数に対する離散フーリエ変換（DFT）を行う。
最も一般的に用いられる関数であり，
順変換・逆変換の両方に対応する。 \\
\hline

\texttt{fftw\_plan\_dft} &
複素数から複素数 &
任意次元のフーリエ変換を行う。
次元数および各方向の格子点数を配列で指定する必要がある。 \\
\hline

\texttt{fftw\_plan\_dft\_r2c\_\{1d,2d,3d\}} &
実数から複素数 &
実関数から複素関数へのフーリエ変換を行う。
実入力を効率的に扱うため，出力は半分の情報のみを保持する形式になる。 \\
\hline

\texttt{fftw\_plan\_dft\_c2r\_\{1d,2d,3d\}} &
複素数から実数 &
複素関数から実関数への逆変換を行う。
通常は \texttt{r2c} の逆操作として用いられる。 \\
\hline

\texttt{fftw\_plan\_r2r\_\{1d,2d,3d\}} &
実数から実数 &
離散コサイン変換（DCT）や離散サイン変換（DST）など，
実数のみを扱う各種変換を行う。変換の種類は別途指定する。 \\
\hline

\end{tabularx}
\end{table}

またFFTWではデータ型に応じて関数の接頭辞が以下のように異なります。

\begin{itemize}
  \item \texttt{fftw\_} : double 型
  \item \texttt{fftwf\_} : float 型
  \item \texttt{fftwl\_} : long double 型
\end{itemize}

例えば，複素数に対する2次元フーリエ変換を行う場合は，
\texttt{fftw\_plan\_dft\_2d} を用います。試みに2次元フーリエ変換を行う関数がどのような構成になっているか見てみましょう。以下は\texttt{fftw_plan plan = fftw_plan_dft_2d(
    Nx, Ny,
    in, out,
    FFTW_FORWARD,
    FFTW_MEASURE
...
chapter2-2.cppにFFTWを用いて実空間表示の関数を波数空間の関数にフーリエ変換するプログラムを示します。実空間表示の関数として、ここでは2次元格子におけるローレンチアン
\begin{equation}
  f(x,y) = \frac{\delta^2}{x^2+y^2+\delta^2}
\end{equation}
を採用します。ここで$\delta > 0$はローレンチアンの幅に対応するパラメータで、値を大きくすると実空間で緩やかに減衰します。また、数値計算では、無限平面 $\mathbb{R}^2$ を直接扱うことはできないため、$x,y$ を有限サイズの正方格子
\begin{equation}
...
図\ref{fig:lorentzian-fft}(a)にはフーリエ変換前の実空間のローレンチアンのカラーマップ、(b)にはフーリエ変換後のローレンチアンのカラーマップを示しています。また図(c)では、$k_y = 0$でスライスしたフーリエ変換の実部と厳密解とを比較しています。数値計算結果と厳密解が合致していることが分かります。
...
\texttt{chapter2-3-4.cpp}ではxをprivate変数として保持しています。各スレッドで\texttt{x += 1}し、全てのスレッドで$x = 1$となっていることがお分かりになると思います。
...
\texttt{chapter2-3-5.cpp}の実行結果を以下に示します。
...
上の実行結果をもとに手法の違いについて述べます。まず\texttt{No reduction (race)} の結果では，期待される値が 50000000 であるにもかかわらず実際の値はそれより小さくなっています。これは複数のスレッドが同時にsharedの変数に対して
\texttt{+=} の操作を行ったことによる競合が原因です。

次に\texttt{Reduction} を用いた場合、各スレッドがそれぞれ独立したローカル変数を用いて加算を行い、並列処理の終了後にそれらをまとめて合計する仕組みになっているため結果が正しく 50000000 となっています。
この方法ではループの実行中に共有変数を直接更新する必要がないため、競合を回避しつつ効率的に計算を行うことができます。実行時間が非常に短くなっていることからも，オーバーヘッドが小さいことが分かります。

最後に比較のため\texttt{Atomic}を用いた場合について述べます。Atomicは並列化こそしていますが、各スレッド1つずつ順序立てて実行するプログラムとなっています。結果は正しくなっていますが、実行時間が大幅に長くなっています。

以上より並列ループ内で総和を計算するような場合には\texttt{reduction} を用いることで正確性と効率の両方を満たすことができることが分かります。
...
\begin{figure}[t] % 位置: t=top, b=bottom, h=here(強くは効かない), p=page
  \centering
  \includegraphics[width=0.95\linewidth]{Oncampus-Fig5-v1.eps}
  \caption{正方格子の模式図。$t$が最近接ホッピングを、$t'$が次近接ホッピングを表す。格子定数は$a = 1$としている。}
  \label{fig:chiRPA}
\end{figure}
